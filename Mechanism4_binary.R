...
Generate_data <- function(N, para){
  ...
  M <- rbinom(N, size = 1, prob = pnorm(para[['alpha_0']] + para[['alpha_z']] * Z + para[['alpha_y']] * Y_com))
...
joint_density <- function(para, data){
  ...
  px ^ x * (1 - px) ^ (1 - x) * 
    ...
    pnorm(para[['theta_0']] + para[['theta_z']] * z + para[["theta_x"]] * x) ^ y * 
    (pnorm(para[['theta_0']] + para[['theta_z']] * z + para[["theta_x"]] * x, lower.tail = FALSE)) ^ (1 - y) *
    (pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_z']] * z)) ^ m *
    (pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_z']] * z, lower.tail = FALSE)) ^ (1 - m)
...
score_fun <- function(para, data){
  ...
  result <- z * log(pz) + (1 - z) * log(1 - pz) +
    ...
    y * log(pnorm(para[['theta_0']] + para[['theta_z']] * z + para[["theta_x"]] * x)) +
    (1 - y) * log(pnorm(para[['theta_0']] + para[['theta_z']] * z + para[["theta_x"]] * x, lower.tail = FALSE)) +
    m * log(pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_z']] * z)) +
    (1 - m) * log(pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_z']] * z, lower.tail = FALSE))
...
N <- 1000
parameters <- list(prop_x = 0.5, prop_z = 0.5, theta_0 = -1,  theta_z = 2 , theta_x = 1,  alpha_0 = 1,  alpha_y = -1, alpha_z = 1.5)
# true CRR1_0 : pnorm(parameters[['theta_0']] + parameters[['theta_z']] + parameters[['theta_x']]) *  pnorm(parameters[['theta_0']]) / (pnorm(parameters[['theta_0']] + parameters[['theta_z']]) *  pnorm(parameters[['theta_0']] + parameters[['theta_x']]))
# true COR1_0 : pnorm(parameters[['theta_0']] + parameters[['theta_z']] + parameters[['theta_x']]) *  pnorm(parameters[['theta_0']]) / (pnorm(parameters[['theta_0']] + parameters[['theta_z']]) *  pnorm(parameters[['theta_0']] + parameters[['theta_x']])) / 
#               (pnorm(parameters[['theta_0']] + parameters[['theta_z']] + parameters[['theta_x']], lower.tail = FALSE) *  pnorm(parameters[['theta_0']], lower.tail = FALSE) / (pnorm(parameters[['theta_0']] + parameters[['theta_z']], lower.tail = FALSE) *  pnorm(parameters[['theta_0']] + parameters[['theta_x']], lower.tail = FALSE)))
...
for (i in 1:n) {
  ...
  para <- list(prop_x = px, prop_z = pz, theta_0 = model$coefficients[[1]],  theta_z = model$coefficients[[2]], theta_x = model$coefficients[[3]],  alpha_0 = runif(1, -2, 2),  alpha_z = runif(1, -2, 2), alpha_y = runif(1, -2, 2)) |> unlist()
  
  data_final <- Data_final(data, para, B)
  t <- 0
  CRR1_0 <- pnorm(para[['theta_0']] + para[['theta_z']] + para[['theta_x']]) *  pnorm(para[['theta_0']]) /
    (pnorm(para[['theta_0']] + para[['theta_z']]) *  pnorm(para[['theta_0']] + para[['theta_x']]))
  
  COR1_0 <- CRR1_0 / (pnorm(para[['theta_0']] + para[['theta_z']] + para[['theta_x']], lower.tail = FALSE) *  pnorm(para[['theta_0']], lower.tail = FALSE) /
                        (pnorm(para[['theta_0']] + para[['theta_z']], lower.tail = FALSE) *  pnorm(para[['theta_0']] + para[['theta_x']], lower.tail = FALSE)))
  
  CR <- list(CRR1_0 = CRR1_0, COR1_0 = COR1_0) |> unlist()
  
  temp_CR <- rep(0, 2)
  h <- hy(x = data_final$X, y = data_final$Y, z = data_final$Z, para) 
  
  while(sum((CR - temp_CR) ^ 2) >= 1e-06 & t <= 1000){
    num = 0
    t = t + 1
    temp_CRR1_0 <- pnorm(para[['theta_0']] + para[['theta_z']] + para[['theta_x']]) *  pnorm(para[['theta_0']]) /
      (pnorm(para[['theta_0']] + para[['theta_z']]) *  pnorm(para[['theta_0']] + para[['theta_x']]))
    temp_COR1_0 <- temp_CRR1_0 / (pnorm(para[['theta_0']] + para[['theta_z']] + para[['theta_x']], lower.tail = FALSE) *  pnorm(para[['theta_0']], lower.tail = FALSE) /
                                    (pnorm(para[['theta_0']] + para[['theta_z']], lower.tail = FALSE) *  pnorm(para[['theta_0']] + para[['theta_x']], lower.tail = FALSE)))
    
    temp_CR <- c(temp_CRR1_0, temp_COR1_0)
...
CE_mle <- vector(mode = 'list', length = n)
for (i in 1 : n) {
  set.seed(i)

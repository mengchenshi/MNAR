...
N <- 1000
parameters <- list(prop_x = 0.5, prop_z = 0.5, theta_0 = -1, theta_z = 2, theta_x = -1, sigma_x = 1, alpha_0 = 1, alpha_z = -1, alpha_x = 1.5)

# 填补之后的最终数据，在初始值para下
B <- 50
lo <- rep(c(0.01, -Inf, 0.01, -Inf), c(2, 3, 1, 3))
hi <- rep(c(0.99, Inf, Inf, Inf), c(2, 3, 1, 3))
...
for (i in 1:n) {
  ...
  para <-list(prop_x = px, prop_z = pz, theta_0 = model$coefficients[[1]],  theta_z = model$coefficients[[2]], theta_x = model$coefficients[[3]],
              sigma_x = sd(model$residuals), alpha_0 = runif(1, -5, 5), alpha_z = runif(1, -5, 5), alpha_x = runif(1, -5, 5)) |> unlist()
  ...
  while(sum((para[2:6] - temp_para[2:6])^2) >= 1e-06 & t <= 1000){
    num = 0
    t = t + 1
    temp_para <- para

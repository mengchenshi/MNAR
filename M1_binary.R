...
score_fun <- function(para, data){
  ...
  result <- z * log(pz) + (1 - z) * log(1 - pz) +
    ...
    m * log(pnorm(para[['alpha_0']] + para[['alpha_z']] * z)) +
    (1 - m) * log(pnorm(para[['alpha_0']] + para[['alpha_z']] * z, lower.tail = FALSE))
...
parameters <- list(prop_x = 0.5, prop_z = 0.5, theta_0 = -1,  theta_z = 2, theta_x = 1,  alpha_0 = 1.5,  alpha_z = -1.5)
N <- 1000
#h <- 1
B <- 50

lo <- rep(c(0.01, -10), c(2, 5))
hi <- rep(c(0.99, 10), c(2, 5))
...
for (i in 1:n) {
  ...
  # 填补之后的最终数据，在初始值para下
  para <- list(prop_x = px, prop_z = pz, theta_0 = model$coefficients[[1]],  theta_z = model$coefficients[[2]], theta_x = model$coefficients[[3]],  alpha_0 = runif(1, -5, 5),  alpha_z = runif(1, -5, 5)) |> unlist()

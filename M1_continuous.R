...
Generate_data <- function(N, para){
  ...
  # P(Y = y | x, z)
  Y_com <- rlnorm(N, meanlog = para[['theta_0']] + para[['theta_z']] * Z + para[['theta_x']] * X_com, sdlog = para[['sigma_x']])
  
  M <- rbinom(N, size = 1, prob = pnorm(para[['alpha_0']] + para[['alpha_z']] * Z))
...
hy <- function(x, y, z, para){
  ...
  (1 / (y * sqrt(2 * pi * para[['sigma_x']] ^ 2)) * exp(- (log(y) - (para[['theta_0']] + para[['theta_z']] * z + para[['theta_x']] * x)) ^ 2 / (2 * para[['sigma_x']] ^ 2)))
...
hy_imp <- function(data, para, B, index){
  ...
  Y_imp <- map2(X_imp, Z, \(x, z)  rlnorm(2 * B, meanlog = para[['theta_0']] + para[['theta_z']] * z + para[['theta_x']] * x, sdlog = para[['sigma_x']])) #
...
joint_density <- function(para, data){
  ...
  px ^ x * (1 - px) ^ (1 - x) * 
    pz ^ z * (1 - pz) ^ (1 - z) *
    (1 / (y * sqrt(2 * pi * para[['sigma_x']] ^ 2)) * exp(- (log(y) - (para[['theta_0']] + para[['theta_z']] * z + para[['theta_x']] * x)) ^ 2 / (2 * para[['sigma_x']] ^ 2))) *
    (pnorm(para[['alpha_0']] + para[['alpha_z']] * z)) ^ m *
    (pnorm(para[['alpha_0']] + para[['alpha_z']] * z, lower.tail = FALSE)) ^ (1 - m)
...
score_fun <- function(para, data){
  ...
  result <- z * log(pz) + (1 - z) * log(1 - pz) +
    ...
    (-0.5 * log(para[['sigma_x']] ^ 2) - (log(y) - (para[['theta_0']] + para[['theta_z']] * z + para[['theta_x']] * x)) ^ 2 / (2 * para[['sigma_x']] ^ 2)) +
    m * (pnorm(para[['alpha_0']] + para[['alpha_z']] * z, log.p = TRUE)) +
    (1 - m) * (pnorm(para[['alpha_0']] + para[['alpha_z']] * z, lower.tail = FALSE, log.p = TRUE))
...
N <- 1000
parameters <- list(prop_x = 0.5, prop_z = 0.5, theta_0 = -1, theta_z = 2, theta_x = -1, sigma_x = 1, alpha_0 = 1.8, alpha_z = -1)


B <- 50
lo <- rep(c(0.01, -Inf, 0.01, -Inf), c(2, 3, 1, 2))
hi <- rep(c(0.99, Inf, Inf, Inf), c(2, 3, 1, 2))
...
for (i in 1 : n) {
  ...
  para <-list(prop_x = px, prop_z = pz, theta_0 = model$coefficients[[1]],  theta_z = model$coefficients[[2]], theta_x = model$coefficients[[3]],
              sigma_x = sd(model$residuals)) |> unlist()

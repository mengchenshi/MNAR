...
Generate_data <- function(N, para){
  ...
  M <- rbinom(N, size = 1, prob = pnorm(para[['alpha_0']] + para[['alpha_x']] * X_com + para[['alpha_y']] * Y_com))
...
joint_density <- function(para, data){
  ...
  px ^ x * (1 - px) ^ (1 - x) * 
    ...
    pnorm(para[['theta_0']] + para[['theta_z']] * z + para[["theta_x"]] * x) ^ y * 
    (pnorm(para[['theta_0']] + para[['theta_z']] * z + para[["theta_x"]] * x, lower.tail = FALSE)) ^ (1 - y) *
    (pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_x']] * x)) ^ m *
    (pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_x']] * x, lower.tail = FALSE)) ^ (1 - m)
...
score_fun <- function(para, data){
  ...
  result <- z * log(pz) + (1 - z) * log(1 - pz) +
    ...
    m * (pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_x']] * x, log.p = TRUE)) +
    (1 - m) * (pnorm(para[['alpha_0']] + para[['alpha_y']] * y + para[['alpha_x']] * x, lower.tail = FALSE, , log.p = TRUE))
...
N <- 1000
parameters <- list(prop_x = 0.5, prop_z = 0.5, theta_0 = -1 ,  theta_z = 2 , theta_x = 1,  alpha_0 = 1.5,  alpha_y = 1.5, alpha_x = -2)
...
for (i in 1 : n) {
  ...
  para <- list(prop_x = px, prop_z = pz, theta_0 = model$coefficients[[1]],  theta_z = model$coefficients[[2]], theta_x = model$coefficients[[3]], 
               alpha_0 = runif(1, -5, 5), alpha_y = runif(1, -5, 5), alpha_x = runif(1, -5, 5)) |> unlist()
  lo <- rep(c(0.01, -Inf), c(2, 6))
  hi <- rep(c(0.99, Inf), c(2, 6))

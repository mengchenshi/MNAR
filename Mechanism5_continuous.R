...
Generate_data <- function(N, para){
  ...
  # P(Y = y | x, z)
  Y_com <- I(X_com == 1) * rnorm(N, mean = para[['theta_10']] + para[['theta_1z']] * Z, sd = para[['sigma_x1']]) +
    I(X_com == 0) * rnorm(N, mean = para[['theta_00']] + para[['theta_0z']] * Z, sd = para[['sigma_x0']])
  
  M <- I(X_com == 1) * rbinom(N, size = 1, prob = pnorm(((para[['alpha_0']] + Z * para[['alpha_z']])  + para[['beta']] * (Y_com - (para[['theta_10']] +  para[['theta_1z']] * Z))) / para[['sigma_x1']])) +
    I(X_com == 0) * rbinom(N, size = 1, prob = pnorm(((para[['alpha_0']] + Z * para[['alpha_z']]) + para[['beta']] * (Y_com - (para[['theta_00']] + para[['theta_0z']] * Z))) / para[['sigma_x0']]))
...
score_fun <- function(para, data){
  ...
  # 确保概率参数在合理范围内
  px <- max(min(px, 1 - 1e-10), 1e-10)
  pz <- max(min(pz, 1 - 1e-10), 1e-10)
  
  # 确保标准差参数为正
  sigma_x1 <- max(para[['sigma_x1']], 1e-10)
  sigma_x0 <- max(para[['sigma_x0']], 1e-10)
  
  result <-  m * (x * (log(px) + (-0.5 * log(para[['sigma_x1']] ^ 2) - (y - (para[['theta_10']] + para[['theta_1z']] * z)) ^ 2 / (2 * para[['sigma_x1']] ^ 2)) + log(pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_10']] + para[['theta_1z']] * z))) / para[['sigma_x1']])))+ 
                    (1 - x) * (log(1 - px) + (-0.5 * log(para[['sigma_x0']] ^ 2) - (y - (para[['theta_00']] + para[['theta_0z']] * z)) ^ 2 / (2 * para[['sigma_x0']] ^ 2)) + log(pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_00']] + para[['theta_0z']] * z))) / para[['sigma_x0']])))) +
    (1 - m) * (x * (log(px) + (-0.5 * log(para[['sigma_x1']] ^ 2) - (y - (para[['theta_10']] + para[['theta_1z']] * z)) ^ 2 / (2 * para[['sigma_x1']] ^ 2)) + log(pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_10']] + para[['theta_1z']] * z))) / para[['sigma_x1']], lower.tail = FALSE)))+ 
                 (1 - x) * (log(1 - px) + (-0.5 * log(para[['sigma_x0']] ^ 2) - (y - (para[['theta_00']] + para[['theta_0z']] * z)) ^ 2 / (2 * para[['sigma_x0']] ^ 2)) + log(pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_00']] + para[['theta_0z']] * z))) / para[['sigma_x0']], lower.tail = FALSE)))) + 
    z * log(pz) + (1 - z) * log(1 - pz)
  
  
  # result <- z * log(pz) + (1 - z) * log(1 - pz) +
  #   x * log(px) + (1 - x) * log(1 - px) +
  #   x * (-0.5 * log(sigma_x1 ^ 2) - (y - (para[['theta_10']] + para[['theta_1z']] * z)) ^ 2 / (2 * sigma_x1 ^ 2)) +
  #   (1 - x) * (-0.5 * log(sigma_x0 ^ 2) - (y - (para[['theta_00']] + para[['theta_0z']] * z)) ^ 2 / (2 * sigma_x0 ^ 2)) +
  #   m * log(pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_10']] + para[['theta_1z']] * z))) / sigma_x1) ^ x *
  #             pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_00']] + para[['theta_0z']] * z))) / sigma_x0) ^ (1 - x)) +
  #   (1 - m) * log(pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_10']] + para[['theta_1z']] * z))) / sigma_x1, lower.tail = FALSE) ^ x *
  #                   pnorm(((para[['alpha_0']] + z * para[['alpha_z']]) + para[['beta']] * (y - (para[['theta_00']] + para[['theta_0z']] * z))) / sigma_x0, lower.tail = FALSE) ^ (1 - x))
  
  if (any(is.na(result) | is.infinite(result))) {
    return(-Inf) # 如果计算结果非法，返回极小值
  }
...
N <- 1000
parameters <- list(prop_x = 0.5, prop_z = 0.5,theta_10 = 1, theta_1z = 2, theta_00 = -1, theta_0z = 1.5, sigma_x1 = 1, sigma_x0 = 2, alpha_0 = 1.5, alpha_z = 1.5, beta = 1)
...
for (i in 1 : n) {
  ...
  while(sum((para - temp_para)^2) >= 1e-06 & t <= 1000){
    num = 0
    t = t + 1
    temp_para <- para
...
for (i in 1 : n) {
  ...
  para <-list(prop_x = px, prop_z = pz, theta_10 = model_x1$coefficients[[1]], theta_1z = model_x1$coefficients[[2]],theta_00 = model_x0$coefficients[[1]],
              theta_0z = model_x0$coefficients[[2]], sigma_x1 = sd(model_x1$residuals), sigma_x0 = sd(model_x0$residuals)
              ) |> unlist()
